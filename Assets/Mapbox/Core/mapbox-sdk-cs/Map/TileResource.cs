    ...
    public sealed class TileResource : IResource
    {
        ...
        public static TileResource MakeRaster(CanonicalTileId id, string styleUrl)
        {
            return new TileResource(
                string.Format(
                    "{0}/{1}",
                    MapUtils.NormalizeStaticStyleURL(
        ...
        public static TileResource MakeClassicRaster(CanonicalTileId id, string tilesetId)
        {
            return new TileResource(
                string.Format(
                    "{0}/{1}.png",
                    MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.satellite"),
                    id
                )
            );
        }
        ...
        public static TileResource MakeRawPngRaster(CanonicalTileId id, string tilesetId)
        {
            return new TileResource(
                string.Format(
                    "{0}/{1}.pngraw",
                    MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.terrain-rgb"),
                    id
                )
            );
        }
        ...
        public static TileResource MakeVector(CanonicalTileId id, string tilesetId)
        {
            return new TileResource(
                string.Format(
                    "{0}/{1}.vector.pbf",
                    MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.mapbox-streets-v7"),
                    id
                )
            );
        }

        internal static TileResource MakeStyleOptimizedVector(
            CanonicalTileId id,
            string tilesetId,
            string optimizedStyleId,
            string modifiedDate
        )
        {
            return new TileResource(
                string.Format(
                    "{0}/{1}.vector.pbf?style={2}@{3}",
                    MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.mapbox-streets-v7"),
                    id,
                    optimizedStyleId,
                    modifiedDate
