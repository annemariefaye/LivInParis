namespace Mapbox.MapboxSdkCs.UnitTest
{
    ...
    internal class SQLiteCacheTest
    {
        ...
        public void InsertSameTileNoOverwrite()
        {
            ...
            Assert.AreEqual(
                1,
                _cache.TileCount(TS_NO_OVERWRITE),
                "tileset {0}: unexpected number of tiles",
                TS_NO_OVERWRITE
            );
        }
        ...
        public void InsertSameTileForceOverwrite()
        {
            ...
            Assert.AreEqual(
                1,
                _cache.TileCount(TS_FORCE_OVERWRITE),
                "tileset {0}: unexpected number of tiles",
                TS_FORCE_OVERWRITE
            );
        }
        ...
        public IEnumerator ConcurrentTilesetInsert()
        {
            ...
            while (
                Runnable.IsRunning(rIdCr1)
                || Runnable.IsRunning(rIdCr2)
                || Runnable.IsRunning(rIdCr3)
                || Runnable.IsRunning(rIdCr4)
            )
            {
                yield return null;
            }
        ...
        public void VerifyTilesFromConcurrentInsert()
        {
            ...
            string[] tilesetNames = new string[]
            {
                TS_CONCURRENT1,
                TS_CONCURRENT2,
                TS_CONCURRENT3,
            ...
            foreach (string tilesetName in tilesetNames)
            {
                Assert.AreEqual(
                    _tileIds.Count,
                    _cache.TileCount(tilesetName),
                    "tileset '{0}' does not contain expected number of tiles",
                    tilesetName
                );
            }
        ...
        public void Prune()
        {
            ...
            Assert.AreEqual(
                _maxCacheTileCount,
                _cache.TileCount(TS_PRUNE),
                _cache.PruneCacheDelta,
                "tileset [{0}]: pruning did not work as expected",
                TS_PRUNE
            );
        }
        ...
        public void Clear()
        {
            ...
            Assert.GreaterOrEqual(
                tileCnt,
                _cache.MaxCacheSize,
                "number of tiles lower than expected"
            );
        ...
        private List<long> simpleInsert(
            string tileSetName,
            bool forceInsert,
            HashSet<CanonicalTileId> tileIds = null,
            int itemCount = 1000
        )
        {
            ...
            for (int i = 0; i < itemCount; i++)
            {
                CanonicalTileId tileId =
                    null != tileIds ? tileIds.ElementAt(i) : new CanonicalTileId(0, 0, 0);
