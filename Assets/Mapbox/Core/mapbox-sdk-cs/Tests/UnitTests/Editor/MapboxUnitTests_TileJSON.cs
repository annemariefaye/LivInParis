    ...
    internal class TileJSONTest
    {
        ...
        public IEnumerator MapboxStreets()
        {
            ...
            IEnumerator enumerator = (
                (FileSource)Unity.MapboxAccess.Instance.TileJSON.FileSource
            ).WaitForAllRequests();
        ...
        )
        {
            ...
            Assert.AreEqual(
                boundsSouth,
                response.BoundsParsed.South,
                0.003,
                "Bounds.South does not match"
            );
            Assert.AreEqual(boundsEast, response.BoundsParsed.East, "Bounds.East does not match");
            Assert.AreEqual(
                boundsNorth,
                response.BoundsParsed.North,
                0.003,
                "Bounds.North does not match"
            );
