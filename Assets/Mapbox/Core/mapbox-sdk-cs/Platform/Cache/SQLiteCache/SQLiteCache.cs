namespace Mapbox.Platform.Cache
{
    ...
    public class SQLiteCache : ICache, IDisposable
    {
        ...
        private void init()
        {
            ...
            List<SQLiteConnection.ColumnInfo> colInfoTileset = _sqlite.GetTableInfo(
                typeof(tilesets).Name
            );
            if (0 == colInfoTileset.Count)
            {
                string cmdCreateTableTilesets =
...
name  STRING  NOT NULL
            ...
            List<SQLiteConnection.ColumnInfo> colInfoTiles = _sqlite.GetTableInfo(
                typeof(tiles).Name
            );
            if (0 == colInfoTiles.Count)
            {
                string cmdCreateTableTiles =
                    @"CREATE TABLE tiles(
...
lastmodified INTEGER,
	PRIMARY KEY(
...
#if MAPBOX_DEBUG_CACHE
            string methodName =
                _className + "." + new System.Diagnostics.StackFrame().GetMethod().Name;
            ...
            try
            {
                ...
                if (tilesetId < 0)
                {
                    Debug.LogErrorFormat(
                        "could not get tilesetID for [{0}] tile: {1}",
                        tilesetName,
                        tileId
                    );
                    return;
                ...
                int rowsAffected = _sqlite.InsertOrReplace(
                    new tiles
                    {
                ...
                if (1 != rowsAffected)
                {
                    throw new Exception(
                        string.Format(
                            "tile [{0} / {1}] was not inserted, rows affected:{2}",
                            tilesetName,
                            tileId,
                            rowsAffected
                        )
        ...
        public CacheItem Get(string tilesetName, CanonicalTileId tileId)
        {
            ...
            {
                Debug.LogErrorFormat(
                    "error getting tile {1} {2} from cache{0}{3}",
                    Environment.NewLine,
                    tilesetName,
                    tileId,
                    ex
        ...
        private int insertTileset(string tilesetName)
        {
            try
            {
                ...
                if (1 != rowsAffected)
                {
                    throw new Exception(
                        string.Format(
                            "tileset [{0}] was not inserted, rows affected:{1}",
                            tilesetName,
                            rowsAffected
                        )
                    );
        ...
        public long TileCount(string tilesetName)
        {
            ...
            return _sqlite.Table<tiles>().Where(t => t.tile_set == tilesetId.Value).LongCount();
        }
