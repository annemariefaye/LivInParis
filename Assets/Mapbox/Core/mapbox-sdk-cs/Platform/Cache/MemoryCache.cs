    ...
    public class MemoryCache : ICache
    {
        ...
        public void Add(string mapdId, CanonicalTileId tilesetId, CacheItem item, bool forceInsert)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    _cachedResponses.Remove(
                        _cachedResponses.OrderBy(c => c.Value.AddedToCacheTicksUtc).First().Key
                    );
                }
        ...
        public void Clear(string tilesetId)
        {
            ...
            {
                tilesetId += "||";
                List<string> toDelete = _cachedResponses
                    .Keys.Where(k => k.Contains(tilesetId))
                    .ToList();
