    ...
    public class Response
    {
        ...
        public string ExceptionsAsString
        {
            get
            {
                ...
                return string.Join(
                    Environment.NewLine,
                    _exceptions.Select(e => e.Message).ToArray()
                );
            }
        }
        ...
        public static Response FromWebResponse(
                ...
                if (null != apiResponse.Headers)
                {
                    ...
                    foreach (var hdr in apiResponse.Headers)
                    {
                        ...
                        if (key.Equals("X-Rate-Limit-Interval", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            key.Equals("X-Rate-Limit-Limit", StringComparison.OrdinalIgnoreCase)
                        )
                        {
                            long limitLimit;
                            if (long.TryParse(val, out limitLimit))
                            ...
                            key.Equals("X-Rate-Limit-Reset", StringComparison.OrdinalIgnoreCase)
                        )
                        {
                            double unixTimestamp;
                            if (double.TryParse(val, out unixTimestamp))
                            {
                                ...
                                response.XRateLimitReset = beginningOfTime
        ...
        )
        {
            ...
            {
                response.AddException(
                    new Exception(string.Format("Status Code {0}", apiResponse.responseCode))
                );
            }
            if (429 == statusCode)
            {
