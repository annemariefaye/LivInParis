namespace Mapbox.Editor
{
    ...
    public class VectorFilterOptionsDrawer : PropertyDrawer
    {
        ...
        bool showFilters
        {
            get { return EditorPrefs.GetBool(objectId + "VectorSubLayerProperties_showFilters"); }
            set { EditorPrefs.SetBool(objectId + "VectorSubLayerProperties_showFilters", value); }
        }

        GUIContent operatorGui = new GUIContent
        {
            text = "Operator",
            tooltip = "Filter operator to apply. ",
        };
        GUIContent numValueGui = new GUIContent
        {
            text = "Num Value",
            tooltip = "Numeric value to match using the operator.  ",
        };
        GUIContent strValueGui = new GUIContent
        {
            text = "Str Value",
            tooltip = "String value to match using the operator.  ",
        };
        GUIContent minValueGui = new GUIContent
        {
            text = "Min",
            tooltip = "Minimum numeric value to match using the operator.  ",
        };
        GUIContent maxValueGui = new GUIContent
        {
            text = "Max",
            tooltip = "Maximum numeric value to match using the operator.  ",
        };

        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            objectId = property.serializedObject.targetObject.GetInstanceID().ToString();
            VectorFilterOptions options = (VectorFilterOptions)
                EditorHelper.GetTargetObjectOfProperty(property);

            showFilters = EditorGUILayout.Foldout(
                showFilters,
                new GUIContent
                {
                    text = "Filters",
                    tooltip = "Filter features in a vector layer based on criterion specified.  ",
                }
            );
            if (showFilters)
            {
        ...
        private void DrawLayerFilter(
            SerializedProperty originalProperty,
            SerializedProperty propertyFilters,
            int index,
        ...
        )
        {
            ...
            EditorGUILayout.LabelField(
                new GUIContent
                {
                    text = "Key",
                    tooltip =
                        "Name of the property to use as key. This property is case sensitive.",
                },
            ...
            var selectedLayerName = originalProperty
                .FindPropertyRelative("_selectedLayerName")
                .stringValue;

            DrawPropertyDropDown(originalProperty, property);

            EditorGUI.BeginChangeCheck();
            filterOperatorProp.enumValueIndex = EditorGUILayout.Popup(
                filterOperatorProp.enumValueIndex,
                filterOperatorProp.enumDisplayNames,
                GUILayout.MaxWidth(150)
            );
            if (EditorGUI.EndChangeCheck())
            {
        ...
        )
        {
            ...
            if (
                string.IsNullOrEmpty(selectedLayerName)
            ...
            if (_propertyNamesList.Contains(propertyString))
            {
                ...
                for (int extIdx = 0; extIdx < _propertyNamesList.Count; extIdx++)
                {
                    var parsedPropertyString = _propertyNamesList[extIdx]
                        .Split(
                            new string[] { tileJsonData.optionalPropertiesString },
                    ...
                    _propertyNameContent[extIdx] = new GUIContent
                    {
                        ...
                        tooltip = tileJsonData.LayerPropertyDescriptionDictionary[
                            selectedLayerName
                ...
                //display popup
                EditorGUI.BeginChangeCheck();
                _propertyIndex = EditorGUILayout.Popup(
                    _propertyIndex,
                    _propertyNameContent,
                    GUILayout.MaxWidth(150)
                );
                if (EditorGUI.EndChangeCheck())
                {
                ...
                parsedString = _propertyNamesList[_propertyIndex]
                    ...
                    .Trim();
                descriptionString = tileJsonData.LayerPropertyDescriptionDictionary[
                    selectedLayerName
                ][parsedString];
            }
            else
        ...
        private void DrawWarningMessage()
        {
            ...
            EditorGUILayout.LabelField(
                new GUIContent(),
                new GUIContent("No properties"),
                labelStyle,
                new GUILayoutOption[] { GUILayout.MaxWidth(155) }
            );
            return;
        }
    }
