namespace Mapbox.Editor
{
    ...
    public class ModelingSectionDrawer
    {
        ...
        bool showModeling
        {
            get { return EditorPrefs.GetBool(objectId + "VectorSubLayerProperties_showModeling"); }
            set { EditorPrefs.SetBool(objectId + "VectorSubLayerProperties_showModeling", value); }
        }
        static float _lineHeight = EditorGUIUtility.singleLineHeight;

        public void DrawUI(
            SerializedProperty subLayerCoreOptions,
        ...
        )
        {
            ...
            if (showModeling)
            {
                ...
                {
                    GUILayout.Space(-_lineHeight);
                    var lineGeometryOptions = layerProperty.FindPropertyRelative(
                        "lineGeometryOptions"
                    );
                ...
                )
                {
                    ...
                    var snapToTerrainProperty = subLayerCoreOptions.FindPropertyRelative(
                        "snapToTerrain"
                    );
                    snapToTerrainProperty.boolValue = EditorGUILayout.Toggle(
                        snapToTerrainProperty.displayName,
                ...
                if (primitiveTypeProp != VectorPrimitiveType.Point)
                {
                    ...
                    combineMeshesProperty.boolValue = EditorGUILayout.Toggle(
                        combineMeshesProperty.displayName,
                ...
                )
                {
                    ...
                    var colliderOptionsProperty = layerProperty.FindPropertyRelative(
                        "colliderOptions"
                    );
                    EditorGUI.BeginChangeCheck();
