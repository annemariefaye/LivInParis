namespace Mapbox.Editor
{
    ...
    public class ElevationLayerPropertiesDrawer : PropertyDrawer
    {
        ...
        bool ShowPosition
        {
            get { return EditorPrefs.GetBool(objectId + "ElevationLayerProperties_showPosition"); }
        ...
        string CustomSourceTilesetId
        {
            get
            {
                return EditorPrefs.GetString(
                    objectId + "ElevationLayerProperties_customSourceTilesetId"
                );
            }
            set
            {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUILayout.PropertyField(
                property.FindPropertyRelative("elevationLayerType"),
                new GUIContent
                {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                EditorHelper.CheckForModifiedProperty(
                    property.FindPropertyRelative("colliderOptions")
                );
            }
            ...
            if (ShowPosition)
            {
                ...
                EditorGUILayout.PropertyField(
                    property.FindPropertyRelative("modificationOptions"),
                    true
                );

                EditorGUILayout.PropertyField(
                    property.FindPropertyRelative("sideWallOptions"),
                    true
                );

                EditorGUILayout.PropertyField(
                    property.FindPropertyRelative("unityLayerOptions"),
                    true
                );
