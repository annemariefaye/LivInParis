...
public class ScriptablePaletteEditor : Editor
{
    ...
    private const string _HUE_RANGE_INFO =
        "Maximum range that randomly generated colors will deviate from the key color hue";
    private const string _SAT_RANGE_INFO =
        "Maximum range that randomly generated colors will deviate from the key color saturation";
    private const string _VAL_RANGE_INFO =
        "Maximum range that randomly generated colors will deviate from the key color value";
    ...
    public override void OnInspectorGUI()
    {
        ...
        wrapTextStyle.normal.textColor = new Color(
            _HELPER_TEXT_COLOR_VAL,
            _HELPER_TEXT_COLOR_VAL,
            _HELPER_TEXT_COLOR_VAL,
            1.0f
        );
        ...
        EditorGUILayout.LabelField(
            "Generate a palette for building colorization by defining a key color, palette size and hue/saturation/value range parameters.",
            wrapTextStyle
        );
        ...
        EditorGUILayout.LabelField(
            "Base and detail layer colors will be randomly set from the generated palette at runtime for each building.",
            wrapTextStyle
        );
        ...
        sp.m_numColors = EditorGUILayout.IntSlider(
            numColorContent,
            sp.m_numColors,
            _NUM_COLORS_MIN,
            _NUM_COLORS_MAX
        ...
        EditorGUILayout.LabelField(
            "Layer colors can be overriden by enabling any of the overrides below.",
            wrapTextStyle
        );
        EditorGUILayout.LabelField(
            "If a layer's color is overridden, it will be set directly from the defined override color.",
            wrapTextStyle
        );

        EditorGUILayout.Space();

        sp.m_setBaseColor_Override = EditorGUILayout.Toggle(
            "Override base",
            sp.m_setBaseColor_Override
        );
        if (sp.m_setBaseColor_Override)
        {
            sp.m_baseColor_Override = EditorGUILayout.ColorField(
                "Base Color:",
                sp.m_baseColor_Override
            );
        }

        sp.m_setDetailColor1_Override = EditorGUILayout.Toggle(
            "Override detail 1",
            sp.m_setDetailColor1_Override
        );
        if (sp.m_setDetailColor1_Override)
        {
            sp.m_detailColor1_Override = EditorGUILayout.ColorField(
                "Detail 1 Color:",
                sp.m_detailColor1_Override
            );
        }

        sp.m_setDetailColor2_Override = EditorGUILayout.Toggle(
            "Override detail 2",
            sp.m_setDetailColor2_Override
        );
        if (sp.m_setDetailColor2_Override)
        {
            sp.m_detailColor2_Override = EditorGUILayout.ColorField(
                "Detail 2 Color:",
                sp.m_detailColor2_Override
            );
        }
    }
}
