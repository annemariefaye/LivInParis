namespace Mapbox.Editor
{
    ...
    internal class FeatureSubLayerTreeView : TreeViewWithTreeModel<FeatureTreeElement>
    {
        ...
        public FeatureSubLayerTreeView(
            TreeViewState state,
            MultiColumnHeader multicolumnHeader,
            TreeModel<FeatureTreeElement> model,
            int uniqueIdentifier = 3000
        )
        ...
        protected override void RenameEnded(RenameEndedArgs args)
        {
            ...
            if (args.acceptedRename)
            {
                ...
                element.name = string.IsNullOrEmpty(args.newName.Trim())
                    ? args.originalName
                    : args.newName;
                element.Name = string.IsNullOrEmpty(args.newName.Trim())
                    ? args.originalName
        ...
        public void AddElementToTree(SerializedProperty subLayer)
        {
            ...
            var type = (
                (PresetFeatureType)subLayer.FindPropertyRelative("presetFeatureType").enumValueIndex
            ).ToString();
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            for (int i = 0; i < args.GetNumVisibleColumns(); ++i)
            {
                CellGUI(
                    args.GetCellRect(i),
                    rowItem,
                    (FeatureSubLayerColumns)args.GetColumn(i),
                    ref args
                );
            }
        }
        ...
        void CellGUI(
            Rect cellRect,
            TreeViewItem<FeatureTreeElement> item,
            FeatureSubLayerColumns column,
            ref RowGUIArgs args
        )
        {
            ...
            if (column == FeatureSubLayerColumns.Name)
            {
                ...
                {
                    VectorSubLayerProperties vectorSubLayerProperties = (VectorSubLayerProperties)
                        EditorHelper.GetTargetObjectOfProperty(layer);
            ...
            if (column == FeatureSubLayerColumns.Type)
            {
                ...
                var typeString = (
                    (PresetFeatureType)layer.FindPropertyRelative("presetFeatureType").intValue
                ).ToString();
