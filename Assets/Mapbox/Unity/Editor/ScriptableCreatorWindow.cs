namespace Mapbox.Editor
{
    ...
    public class ScriptableCreatorWindow : EditorWindow
    {
        ...
        public static void Open(
            ...
            int index = -1,
            Action<UnityEngine.Object> act = null,
        ...
        void OnGUI()
        {
            ...
            for (int i = 0; i < _assets.Count; i++)
            {
                ...
                var b = Header(
                    string.Format("{0,-40} - {1, -15}", asset.GetType().Name, asset.name),
                    i == activeIndex
                );
                ...
                if (GUILayout.Button(new GUIContent("Select"), header, GUILayout.Width(80)))
                {
                    ...
                    else
                    {
                        if (_index == -1)
                        {
                            _finalize.arraySize++;
                            _finalize
                                .GetArrayElementAtIndex(_finalize.arraySize - 1)
                    ...
                    MapboxDataProperty mapboxDataProperty = (MapboxDataProperty)
                        EditorHelper.GetTargetObjectOfProperty(_container);
            ...
            where T : ScriptableObject
        {
            ...
            {
                path = path.Replace(
                    System.IO.Path.GetFileName(AssetDatabase.GetAssetPath(Selection.activeObject)),
                    ""
                );
            }
