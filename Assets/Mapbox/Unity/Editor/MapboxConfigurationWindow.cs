    ...
    public class MapboxConfigurationWindow : EditorWindow
    {
        ...
        static void Init()
        {
            ...
            _configurationFile = Path.Combine(
                Unity.Constants.Path.MAPBOX_RESOURCES_ABSOLUTE,
                Unity.Constants.Path.CONFIG_FILE
            );
        ...
        static GUIContent[] LoadContent(ScenesList list)
        {
            ...
            {
                outputContent[i] = new GUIContent(
                    content[i].Name,
                    content[i].Image,
                    content[i].ScenePath
                );
        ...
        {
            _showConfigurationFoldout = EditorGUILayout.Foldout(
                _showConfigurationFoldout,
                "Configuration",
                true
            );
            ...
            if (_showConfigurationFoldout)
            {
                ...
                EditorGUI.indentLevel = 2;
                _memoryCacheSize = EditorGUILayout.IntSlider(
                    "Mem Cache Size (# of tiles)",
                    _memoryCacheSize,
                    0,
                    1000
                );
                _fileCacheSize = EditorGUILayout.IntSlider(
                    "File Cache Size (# of tiles)",
                    _fileCacheSize,
                    0,
                    3000
                );
                ...
                _webRequestTimeout = EditorGUILayout.IntField(
                    "Default Web Request Timeout (s)",
                    _webRequestTimeout
                );
        ...
        void DrawPrefabLinks()
        {
            EditorGUI.BeginDisabledGroup(
                _currentTokenStatus != MapboxTokenStatus.TokenValid || _validating
            );
            ...
            else
            {
                EditorGUILayout.LabelField(
                    "Map Prefabs",
                    "Paste your mapbox access token to get started",
                    _titleStyle
                );
            }
            ...
            _selectedPrefab = GUILayout.SelectionGrid(
                -1,
                _prefabContent,
                rowCount,
                _sampleButtonStyle
        ...
        void DrawExampleLinks()
        {
            ...
            _selectedSample = GUILayout.SelectionGrid(
                -1,
                _sampleContent,
                rowCount,
                _sampleButtonStyle
