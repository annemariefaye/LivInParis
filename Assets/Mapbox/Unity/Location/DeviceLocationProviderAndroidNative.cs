namespace Mapbox.Unity.Location
{
    ...
    public class DeviceLocationProviderAndroidNative : AbstractLocationProvider, IDisposable
    {
        ...
        private void getGpsInstance(bool showToastMessages = false)
        {
            ...
            using (
                AndroidJavaClass androidGps = new AndroidJavaClass(
                    "com.mapbox.android.unity.AndroidGps"
                )
            )
            {
                if (null == androidGps)
                ...
                _gpsInstance = androidGps.CallStatic<AndroidJavaObject>(
                    "instance",
                    _activityContext
                );
                if (null == _gpsInstance)
        ...
        private void getSensorInstance()
        {
            ...
            using (
                AndroidJavaClass androidSensors = new AndroidJavaClass(
                    "com.mapbox.android.unity.AndroidSensors"
                )
            )
            {
                ...
                _sensorInstance = androidSensors.CallStatic<AndroidJavaObject>(
                    "instance",
                    _activityContext
                );
                if (null == _sensorInstance)
        ...
        private IEnumerator locationRoutine()
        {
            while (true)
            {
                ...
                if (null != _sensorInstance)
                {
                    _currentLocation.DeviceOrientation = _sensorInstance.Call<float>(
                        "getOrientation"
                    );
                }

                bool locationServiceAvailable = _gpsInstance.Call<bool>(
                    "getIsLocationServiceAvailable"
                );
                ...
                try
                {
                    AndroidJavaObject locNetwork = _gpsInstance.Get<AndroidJavaObject>(
                        "lastKnownLocationNetwork"
                    );
                    AndroidJavaObject locGps = _gpsInstance.Get<AndroidJavaObject>(
        ...
        private void populateWithBetterLocation(
            AndroidJavaObject locGps,
            AndroidJavaObject locNetwork
        )
        {
        ...
        private string time2str(AndroidJavaObject loc)
        {
            ...
            DateTime dtPlugin = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).Add(
