    ...
    public class LocationLogReader : LocationLogAbstractBase, IDisposable
    {
        ...
        public IEnumerator<Location> GetLocations()
        {
            while (true)
            {
                ...
                location.IsLocationServiceEnabled = bool.Parse(
                    tokens[(int)LogfileColumns.LocationServiceEnabled]
                );
                location.IsLocationServiceInitializing = bool.Parse(
                    tokens[(int)LogfileColumns.LocationServiceInitializing]
                );
                location.IsLocationUpdated = bool.Parse(
                    tokens[(int)LogfileColumns.LocationUpdated]
                );
                location.IsUserHeadingUpdated = bool.Parse(
                    tokens[(int)LogfileColumns.UserHeadingUpdated]
                );
                ...
                location.Accuracy = float.TryParse(
                    tokens[(int)LogfileColumns.Accuracy],
                    NumberStyles.Any,
                    _invariantCulture,
                    out accuracy
                )
                ...
                location.UserHeading = float.TryParse(
                    tokens[(int)LogfileColumns.UserHeading],
                    NumberStyles.Any,
                    _invariantCulture,
                    out userHeading
                )
                ...
                location.DeviceOrientation = float.TryParse(
                    tokens[(int)LogfileColumns.DeviceOrientation],
                    NumberStyles.Any,
                    _invariantCulture,
                ...
                location.SpeedMetersPerSecond = float.TryParse(
                    tokens[(int)LogfileColumns.Speed],
                    NumberStyles.Any,
                    _invariantCulture,
                    out speed
                )
                    ...
                    : (float?)null;
                bool hasGpsFix;
                location.HasGpsFix = bool.TryParse(
                    tokens[(int)LogfileColumns.HasGpsFix],
                    out hasGpsFix
                )
                    ? hasGpsFix
                    : (bool?)null;
                int satellitesUsed;
                location.SatellitesUsed = int.TryParse(
                    tokens[(int)LogfileColumns.SatellitesUsed],
                    out satellitesUsed
                )
                    ? satellitesUsed
                    : (int?)null;
                int satellitesInView;
                location.SatellitesInView = int.TryParse(
                    tokens[(int)LogfileColumns.SatellitesInView],
                    out satellitesInView
                )
                    ? satellitesInView
                    : (int?)null;
