namespace Mapbox.Unity.Location
{
    ...
    public class DeviceLocationProvider : AbstractLocationProvider
    {
        ...
        {
            [Header(
                "Set 'EditorLocationProvider' to 'DeviceLocationProvider' and connect device with UnityRemote."
            )]
            [SerializeField]
            [Tooltip(
        ...
        protected virtual void Awake()
        {
            ...
            {
                if (
                    null == _editorDebuggingOnly._locationLogFile
                ...
                _locationService = new MapboxLocationServiceMock(
                    _editorDebuggingOnly._locationLogFile.bytes
                );
            }
            else
            {
            ...
            _waitUpdateTime =
                _updateTimeInMilliSeconds < 500
                    ...
                    : new WaitForSeconds((float)_updateTimeInMilliSeconds / 1000.0f);
            ...
            if (null == _deviceOrientationSmoothing)
            {
                _deviceOrientationSmoothing =
                    transform.gameObject.AddComponent<AngleSmoothingNoOp>();
            }
        ...
        IEnumerator PollLocationRoutine()
        {
            ...
            while (!UnityEditor.EditorApplication.isRemoteConnected)
            {
                ...
                Debug.LogWarning(
                    "Remote device not connected via 'Unity Remote'. Waiting ..."
                        + Environment.NewLine
            ...
            {
                Debug.LogError(
                    "DeviceLocationProvider: " + "Timed out trying to initialize location services!"
                );
            ...
            {
                Debug.LogError(
                    "DeviceLocationProvider: " + "Failed to initialize location services!"
                );
                _currentLocation.IsLocationServiceInitializing = false;
                _currentLocation.IsLocationServiceEnabled = false;
                SendLocation(_currentLocation);
            ...
            System.Globalization.CultureInfo invariantCulture = System
                .Globalization
                .CultureInfo
                .InvariantCulture;
            ...
            while (true)
            {
                ...
                double latitude = double.Parse(
                    lastData.latitude.ToString("R", invariantCulture),
                    invariantCulture
                );
                double longitude = double.Parse(
                    lastData.longitude.ToString("R", invariantCulture),
                    invariantCulture
                );
                Vector2d previousLocation = new Vector2d(
                ...
                if (_currentLocation.IsLocationUpdated)
                {
                    if (_lastPositions.Count > 0)
                    {
                        ...
                        CheapRuler cheapRuler = new CheapRuler(
                            _currentLocation.LatitudeLongitude.x,
                ...
                else
                {
                    ...
                    if (distance >= _minDistanceOldestNewestPosition)
                    {
                        ...
                        for (int i = 1; i < _maxLastPositions; i++)
                        {
                            ...
                            // +90.0 to make top (north) 0é
                            double heading =
                                (Math.Atan2(latDiff, lngDiff) * 180.0 / Math.PI) + 90.0f;
                            // stay within [0..360]é range
                            if (heading < 0)
                            {
                                heading += 360;
                            }
                        ...
                        //fix heading to have 0é for north, 90é for east, 180é for south and 270é for west
                        finalHeading =
                            finalHeading >= 180.0f ? finalHeading - 180.0f : finalHeading + 180.0f;
