        ...
        public void ProcessTileJSONData(TileJSONResponse tjr)
        {
            ...
            foreach (var layer in tjr.VectorLayers)
            {
                ...
                foreach (var property in layer.Fields)
                {
                    ...
                    {
                        if (
                            !LayerPropertyDescriptionDictionary[layerName].ContainsKey(propertyName)
                        )
                        {
                            LayerPropertyDescriptionDictionary[layerName]
                                .Add(propertyName, propertyDescription);
                        }
                    }
                    else
                    {
                    ...
                    if (PropertyDisplayNames.ContainsKey(layerName))
                    {
                        if (!PropertyDisplayNames[layerName].Contains(propertyName))
                        {
                            ...
                            if (
                                LayerSourcesDictionary[layerName].Count > 1
                                && !string.IsNullOrEmpty(tjr.Source)
                            )
                            {
                                var firstSource = tjr
                                    .Source.Split(
                ...
                if (
                    PropertyDisplayNames.ContainsKey(layerName)
                    && PropertyDisplayNames[layerName].Count > 1
                )
                {
