...
namespace Mapbox.Unity
{
    ...
    public class MapboxAccess : IFileSource
    {
        ...
        public void SetConfiguration(MapboxConfiguration configuration, bool throwExecptions = true)
        {
            ...
            {
                TokenValidator.Retrieve(
                    configuration.GetMapsSkuToken,
                    ...
                    (response) =>
                    {
                        ...
                        if (response.Status != MapboxTokenStatus.TokenValid && throwExecptions)
                        {
                            configuration.AccessToken = string.Empty;
                            Debug.LogError(
                                new InvalidTokenException(response.Status.ToString().ToString())
                            );
                        }
                    }
                );
        ...
        private void LoadAccessToken()
        {
            ...
            {
                TextAsset configurationTextAsset = Resources.Load<TextAsset>(
                    Constants.Path.MAPBOX_RESOURCES_RELATIVE
                );
            ...
            SetConfiguration(
                ConfigurationJSON == null
                    ? null
                    : Mapbox.Json.JsonConvert.DeserializeObject<MapboxConfiguration>(
                        ConfigurationJSON
                    )
            );
#endif
            ...
            get
            {
                if (_geocoder == null)
                {
                    _geocoder = new Geocoder(
                        new FileSource(
                            Instance.Configuration.GetMapsSkuToken,
                            _configuration.AccessToken
                        )
                    );
                }
