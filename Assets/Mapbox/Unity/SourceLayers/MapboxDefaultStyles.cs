namespace Mapbox.Unity.Map
{
    ...
    public class StyleAssetPathBundle
    {
        ...
        {
            string topMaterialName = string.Format(
                "{0}{1}",
                styleName,
                Constants.StyleAssetNames.TOP_MATERIAL_SUFFIX
            );
            string sideMaterialName = string.Format(
                "{0}{1}",
                styleName,
                Constants.StyleAssetNames.SIDE_MATERIAL_SUFFIX
            );
            string atlasInfoName = string.Format(
                "{0}{1}",
                styleName,
                Constants.StyleAssetNames.ALTAS_SUFFIX
            );
            string paletteName =
                (styleName == "Simple")
                    ? samplePaletteName
            ...
            string materialFolderPath = Path.Combine(
                path,
                Constants.Path.MAPBOX_STYLES_MATERIAL_FOLDER
            );
            string atlasFolderPath = Path.Combine(path, Constants.Path.MAPBOX_STYLES_ATLAS_FOLDER);
            string paletteFolderPath = Path.Combine(
                path,
