...
namespace Mapbox.Unity.Utilities
{
    ...
    public static class Conversions
    {
        ...
        public static Vector2d StringToLatLon(string s)
        {
            ...
            {
                throw new Exception(
                    string.Format("Could not convert latitude to double: {0}", latLonSplit[0])
                );
            }
            ...
            {
                throw new Exception(
                    string.Format("Could not convert longitude to double: {0}", latLonSplit[0])
                );
            }
        ...
        /// </code>
        /// </example>
        public static Vector2d GeoToWorldPosition(
            double lat,
            double lon,
            Vector2d refPoint,
            float scale = 1
        ...
        public static RectD TileBounds(Vector2 tileCoordinate, int zoom)
        {
            var min = PixelsToMeters(
                new Vector2d(tileCoordinate.x * TileSize, tileCoordinate.y * TileSize),
                zoom
            );
            var max = PixelsToMeters(
                new Vector2d((tileCoordinate.x + 1) * TileSize, (tileCoordinate.y + 1) * TileSize),
                zoom
            );
        ...
        {
            var min = PixelsToMeters(
                new Vector2d(unwrappedTileId.X * TileSize, unwrappedTileId.Y * TileSize),
                unwrappedTileId.Z
            );
            var max = PixelsToMeters(
                new Vector2d(
        ...
        public static UnwrappedTileId LatitudeLongitudeToTileId(
            double latitude,
            double longitude,
            int zoom
        )
        ...
        public static Vector2 LatitudeLongitudeToVectorTilePosition(
            Vector2d coordinate,
            int tileZoom,
        ...
        )
        {
            ...
            var vectorTilePoint = new Vector2(
        ...
        public static Vector2 LatitudeLongitudeToUnityTilePosition(
            Vector2d coordinate,
            UnityTile tile,
            ulong layerExtent = 4096
        )
        {
            return LatitudeLongitudeToUnityTilePosition(
                coordinate,
                tile.CurrentZoom,
                tile.TileScale,
                layerExtent
        ...
        )
        {
            ...
            //vectortile space point (0 - layerExtent)
            var vectorTilePoint = LatitudeLongitudeToVectorTilePosition(
                coordinate,
                tileZoom,
                layerExtent
            );
            ...
            //UnityTile space
            var unityTilePoint = new Vector2(
                (float)(vectorTilePoint.x / layerExtent * _rect.Size.x - (_rect.Size.x / 2))
                    * tileScale,
                (float)(
                    (layerExtent - vectorTilePoint.y) / layerExtent * _rect.Size.y
                    - (_rect.Size.y / 2)
                ) * tileScale
            );
        ...
        {
            return (float)(
                40075016.685578d * Math.Cos(Mathf.Deg2Rad * latitude) / Math.Pow(2f, zoom + 8)
            );
        }
        ...
        {
            return (float)(
                -10000 + ((color.r * 255 * 256 * 256 + color.g * 255 * 256 + color.b * 255) * 0.1)
            );
        }
        ...
        {
            var t = new Vector2(
                (int)Math.Ceiling(p.x / (double)TileSize) - 1,
                (int)Math.Ceiling(p.y / (double)TileSize) - 1
            );
            return t;
        }
    }
