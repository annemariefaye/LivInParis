...
namespace Mapbox.Unity.MeshGeneration.Interfaces
{
    ...
    public class VectorLayerVisualizer : LayerVisualizerBase
    {
        ...
        public T FindMeshModifier<T>()
            where T : MeshModifier
        {
            MeshModifier mod = _defaultStack.MeshModifiers.FirstOrDefault(x =>
                x.GetType() == typeof(T)
            );
            return (T)mod;
        }

        public T FindGameObjectModifier<T>()
            where T : GameObjectModifier
        {
            GameObjectModifier mod = _defaultStack.GoModifiers.FirstOrDefault(x =>
                x.GetType() == typeof(T)
            );
            return (T)mod;
        }

        public T AddOrCreateMeshModifier<T>()
            where T : MeshModifier
        {
            MeshModifier mod = _defaultStack.MeshModifiers.FirstOrDefault(x =>
                x.GetType() == typeof(T)
        ...
        public T AddOrCreateGameObjectModifier<T>()
            where T : GameObjectModifier
        {
            GameObjectModifier mod = _defaultStack.GoModifiers.FirstOrDefault(x =>
                x.GetType() == typeof(T)
        ...
        private void UpdateVector(object sender, System.EventArgs eventArgs)
        {
            ...
            {
                layerUpdateArgs.property.PropertyHasChanged -= layerUpdateArgs
                    .modifier
                    .UpdateModifier;
        ...
        public override void SetProperties(VectorSubLayerProperties properties)
        {
            ...
            else
            {
                if (_defaultStack == null)
                {
                    ...
                    ((ModifierStack)_defaultStack).moveFeaturePositionTo =
                        _layerProperties.moveFeaturePositionTo;
                }
                ...
                case VectorPrimitiveType.Polygon:
                {
                    ...
                    if (_layerProperties.extrusionOptions.extrusionType != Map.ExtrusionType.None)
                    {
                        ...
                        if (useTextureSideWallModifier)
                        {
                            ...
                            GeometryExtrusionWithAtlasOptions atlasOptions =
                                new GeometryExtrusionWithAtlasOptions(
                                    _layerProperties.extrusionOptions,
                                    uvModOptions
                                );
                    ...
                    )
                    {
                        ...
                        colorPaletteMod.m_scriptablePalette =
                            (isCustomStyle)
        ...
        private bool IsFeatureEligibleAfterFiltering(
        ...
        private VectorFeatureUnity GetFeatureinTileAtIndex(
            int index,
            UnityTile tile,
            VectorLayerVisualizerProperties layerProperties
        )
        ...
        private bool ShouldSkipProcessingFeatureWithId(
            ulong featureId,
            UnityTile tile,
            VectorLayerVisualizerProperties layerProperties
        )
        ...
        {
            get { return _layerProperties.coreOptions.isActive; }
        }
        ...
        )
        {
            ...
            _activeCoroutines[tile]
                .Add(Runnable.Run(ProcessLayer(layer, tile, tile.UnwrappedTileId, callback)));
        }

        protected IEnumerator ProcessLayer(
            VectorTileLayer layer,
            UnityTile tile,
        ...
        )
        {
            ...
            VectorLayerVisualizerProperties tempLayerProperties =
                new VectorLayerVisualizerProperties();
            ...
            tempLayerProperties.layerFeatureFilters = _layerProperties
                .filterOptions.filters.Select(m => m.GetFilterComparer())
                .ToArray();
            ...
            switch (_layerProperties.filterOptions.combinerType)
            {
                case Filters.LayerFilterCombinerOperationType.Any:
                    tempLayerProperties.layerFeatureFilterCombiner =
                        Filters.LayerFilterComparer.AnyOf(tempLayerProperties.layerFeatureFilters);
                    break;
                case Filters.LayerFilterCombinerOperationType.All:
                    tempLayerProperties.layerFeatureFilterCombiner =
                        Filters.LayerFilterComparer.AllOf(tempLayerProperties.layerFeatureFilters);
                    break;
                case Filters.LayerFilterCombinerOperationType.None:
                    tempLayerProperties.layerFeatureFilterCombiner =
                        Filters.LayerFilterComparer.NoneOf(tempLayerProperties.layerFeatureFilters);
                    break;
                default:
                    break;
            }
            ...
            tempLayerProperties.buildingsWithUniqueIds =
                (_layerProperties.honorBuildingIdSetting)
                && _layerProperties.buildingsWithUniqueIds;
            ...
            var featureCount =
                (tempLayerProperties.vectorTileLayer == null)
                    ? 0
        ...
        )
        {
            ...
            if (layerProperties.buildingsWithUniqueIds == true) //ids from building dataset is big ulongs
            {
                ...
                if (
                    geom[0][0].X < 0
                    || geom[0][0].X > layerExtent
                    || geom[0][0].Y < 0
                    || geom[0][0].Y > layerExtent
                )
                {
                    return false;
                }
                ...
                )
                {
                    switch (layerProperties.featureProcessingStage)
                    {
                        ...
                        case FeatureProcessingStage.Process:
                            ...
                            if (
                                ShouldSkipProcessingFeatureWithId(
                                    feature.Data.Id,
                                    tile,
                                    layerProperties
                                )
        ...
        {
            if (
                feature.Properties.ContainsKey("extrude")
                && !bool.Parse(feature.Properties["extrude"].ToString())
            )
                return false;
        ...
        protected void Build(VectorFeatureUnity feature, UnityTile tile, GameObject parent)
        {
            if (
                feature.Properties.ContainsKey("extrude")
                && !Convert.ToBoolean(feature.Properties["extrude"])
            )
                return;
