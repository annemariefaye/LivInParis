...
namespace Mapbox.Unity.MeshGeneration.Interfaces
{
    ...
    public class LocationPrefabsLayerVisualizer : VectorLayerVisualizer
    {
        ...
        {
            get { return SubLayerProperties.coreOptions.isActive; }
        }
        ...
        public override void SetProperties(VectorSubLayerProperties properties)
        {
            ...
            if (item.spawnPrefabOptions.prefab == null)
            {
                ...
                Debug.LogWarning(
                    "No prefab assigned for POI Layer - "
                        + layerName
            ...
            switch (item.coreOptions.geometryType)
            {
                case VectorPrimitiveType.Point:
                    ...
                    if (
                        typeof(PrefabItemOptions)
                            .GetTypeInfo()
                            .IsAssignableFrom(item.GetType().GetTypeInfo())
                    ) //to check that the instance is of type PrefabItemOptions
#else
...
#endif
                    {
                        ...
                        (_defaultStack as ModifierStack).moveFeaturePositionTo =
                            item.moveFeaturePositionTo;
                        ...
                        if (
                            (
                                item.findByType == LocationPrefabFindBy.MapboxCategory
        ...
        {
            string propertyName = "";
            item.categoryPropertyFromFindByTypeDictionary.TryGetValue(
                item.findByType,
                out propertyName
            );
            ...
            if (item.findByType == LocationPrefabFindBy.MapboxCategory)
            {
                List<LocationPrefabCategories> categoriesList = GetSelectedCategoriesList(
                    item.categories
                );
                ...
                else
                {
                    ...
                    {
                        stringsList = LocationPrefabCategoryOptions.GetMakiListFromCategory(
                            category
                        );
        ...
        private void SetDensityFilters(PrefabItemOptions item)
        {
            ...
            string propertyName = "";
            item.densityPropertyFromFindByTypeDictionary.TryGetValue(
                item.findByType,
                out propertyName
            );
        ...
        private void SetNameFilters(PrefabItemOptions item)
        {
            ...
            string propertyName = "";
            item.namePropertyFromFindByTypeDictionary.TryGetValue(
                item.findByType,
                out propertyName
            );
        ...
        private List<LocationPrefabCategories> GetSelectedCategoriesList(
            LocationPrefabCategories selectedCategories
        )
        {
            List<LocationPrefabCategories> containingCategories =
                new List<LocationPrefabCategories>();
            ...
            foreach (object value in eligibleValues)
            {
                ...
                if (
                    category == LocationPrefabCategories.AnyCategory
                    || category == LocationPrefabCategories.None
                )
                {
                    continue;
                }
        ...
        )
        {
            ...
            if (
                (SubLayerProperties as PrefabItemOptions).findByType
        ...
        private void BuildFeatureFromLatLon(VectorTileLayer layer, UnityTile tile)
        {
            if (tile.TileState != Enums.TilePropertyState.Unregistered)
            {
                ...
                for (int i = 0; i < coordinates.Length; i++)
                {
                    ...
                    Mapbox.Map.UnwrappedTileId coordinateTileId =
                    ...
                    if (coordinateTileId.Canonical.Equals(tile.CanonicalTileId))
                    {
                        ...
                        latLonPoint.Add(
                            Conversions
                                .LatitudeLongitudeToUnityTilePosition(
                                    coordinate,
                                    tile.CurrentZoom,
                                    tile.TileScale,
                                    layer.Extent
