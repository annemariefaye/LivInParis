...
namespace Mapbox.Unity.MeshGeneration.Data
{
    ...
    public class AtlasEntity
    {
        ...
        public void CalculateParameters()
        {
            ...
            WallToFloorRatio =
                (1 - TopSectionRatio - BottomSectionRatio)
    ...
    public class AtlasInfo : ScriptableObject
    {
        ...
        protected virtual void OnValidate()
        {
            ...
            if (AtlasEntityType == AtlasEntityType.TextureBased)
            {
                ...
                {
                    tex.FirstFloorHeight =
                        tex.PreferredEdgeSectionLength
                        * (
                            (tex.TextureRect.height * tex.BottomSectionRatio)
                            / tex.TextureRect.width
                        );
                    tex.TopFloorHeight =
                        tex.PreferredEdgeSectionLength
                        * ((tex.TextureRect.height * tex.TopSectionRatio) / tex.TextureRect.width);
                    tex.FloorHeight =
                        tex.PreferredEdgeSectionLength
                        * (
                            (1 - tex.TopSectionRatio - tex.BottomSectionRatio)
                            * (tex.TextureRect.height / tex.TextureRect.width)
                        );
                }
            }
            else
            {
                foreach (var tex in Textures)
                {
                    tex.BottomSectionRatio =
                        (tex.FirstFloorHeight / tex.PreferredEdgeSectionLength)
                        * tex.TextureRect.width
                        / tex.TextureRect.height;
                    tex.TopSectionRatio =
                        (tex.TopFloorHeight / tex.PreferredEdgeSectionLength)
                        * tex.TextureRect.width
                        / tex.TextureRect.height;
                }
            }
