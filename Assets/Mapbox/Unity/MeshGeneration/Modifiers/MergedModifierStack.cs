...
namespace Mapbox.Unity.MeshGeneration.Modifiers
{
    ...
    public class MergedModifierStack : ModifierStackBase
    {
        ...
        private Dictionary<UnityTile, List<MeshData>> _cached =
            new Dictionary<UnityTile, List<MeshData>>();
        ...
        private Dictionary<UnityTile, List<VectorEntity>> _activeObjects =
            new Dictionary<UnityTile, List<VectorEntity>>();
        ...
        protected virtual void OnEnable()
        {
            ...
            _listPool = new ObjectPool<List<VectorEntity>>(() =>
            {
                return new List<VectorEntity>();
            });
            _meshDataPool = new ObjectPool<List<MeshData>>(() =>
            {
            ...
            if (_cached.ContainsKey(tile))
            {
                ...
                for (int i = 0; i < _counter; i++)
                {
                    ...
                    for (int j = 0; j < c2; j++)
                    {
                        ...
                        {
                            _tempMeshData.Triangles.Add(
                                new List<int>(_temp2MeshData.Triangles[j].Count)
                            );
                        }
                    }
