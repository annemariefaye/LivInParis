namespace Mapbox.Unity.MeshGeneration.Modifiers
{
    ...
    public class TextureSideWallModifier : MeshModifier
    {
        ...
        public override void SetProperties(ModifierProperties properties)
        {
            ...
            {
                _options = (
                    (GeometryExtrusionOptions)properties
        ...
        public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
        {
            ...
            _currentFacade = _options.atlasInfo.Textures[
                UnityEngine.Random.Range(0, _options.atlasInfo.Textures.Count)
            ];
            ...
            _singleFloorHeight =
                (tile.TileScale * _currentFacade.FloorHeight) / _currentFacade.MidFloorCount;
            ...
            if (_options.extrusionGeometryType != ExtrusionGeometryType.RoofOnly)
            {
                ...
                finalTopHeight =
                    (height - finalFirstHeight) < _scaledTopFloorHeight ? 0 : _scaledTopFloorHeight;
                ...
                for (int i = 0; i < md.Edges.Count; i += 2)
                {
                    ...
                    while (currentWallLength > _singleColumnLength)
                    {
                        ...
                        var stepRatio =
                            (float)
                                Math.Min(
                                    _currentFacade.ColumnCount,
                                    Math.Floor(currentWallLength / _singleColumnLength)
                                ) / _currentFacade.ColumnCount;
        ...
        private void CreateWall(MeshData md)
        {
            ...
            rightOfEdgeUv =
                _currentTextureRect.xMin + _currentTextureRect.size.x * columnScaleRatio; // Math.Min(1, ((float)(Math.Floor(columnScaleRatio * _currentFacade.ColumnCount) + 1) / _currentFacade.ColumnCount));

            _minWallLength =
                (_scaledPreferredWallLength / _currentFacade.ColumnCount) * _wallSizeEpsilon;
            //common for all top/mid/bottom segments
            wallNormal = new Vector3(
                -(wallSegmentFirstVertex.z - wallSegmentSecondVertex.z),
                0,
                (wallSegmentFirstVertex.x - wallSegmentSecondVertex.x)
            ).normalized;
        ...
        private void LeftOverRow(MeshData md, float leftOver)
        {
            ...
            {
                md.Vertices.Add(
                    new Vector3(wallSegmentFirstVertex.x, currentY1, wallSegmentFirstVertex.z)
                );
                md.Vertices.Add(
                    new Vector3(wallSegmentSecondVertex.x, currentY2, wallSegmentSecondVertex.z)
                );
                //move offsets bottom
                currentY1 -= leftOver;
                currentY2 -= leftOver;
                ...
                else
                {
                    ...
                    md.UV[0]
                        .Add(
                            new Vector2(
                                _currentTextureRect.xMin + _narrowWallWidthDelta,
                                _currentTextureRect.yMax
                            )
                        );
                    md.UV[0]
                        .Add(
            ...
            while (_currentMidHeight >= _singleFloorHeight - 0.01f)
            {
                ...
                _midUvInCurrentStep =
                    (
                        (float)
                            Math.Min(
                                _currentFacade.MidFloorCount,
                                Math.Round(_currentMidHeight / _singleFloorHeight)
                            )
                    ) / _currentFacade.MidFloorCount;
                        ...
                        .Add(
                            new Vector2(
                                ...
                                _currentFacade.topOfMidUv
                                    - _currentFacade.midUvHeight * _midUvInCurrentStep
                            )
                        );
                    md.UV[0]
                        .Add(
        ...
        private void TopFloor(MeshData md, float leftOver)
        {
            ...
            md.Vertices.Add(
                new Vector3(
                    wallSegmentFirstVertex.x,
                    wallSegmentFirstVertex.y - leftOver,
                    wallSegmentFirstVertex.z
                )
            );
            md.Vertices.Add(
                new Vector3(
                    wallSegmentSecondVertex.x,
                    wallSegmentSecondVertex.y - leftOver,
            ...
            md.Vertices.Add(
                new Vector3(
                    wallSegmentFirstVertex.x,
                    wallSegmentFirstVertex.y - leftOver - finalTopHeight,
                    wallSegmentFirstVertex.z
                )
            );
            md.Vertices.Add(
                new Vector3(
                    wallSegmentSecondVertex.x,
                    wallSegmentSecondVertex.y - leftOver - finalTopHeight,
                    wallSegmentSecondVertex.z
                )
            );
        ...
        private void FirstFloor(MeshData md, float hf)
        {
            md.Vertices.Add(
                new Vector3(
                    wallSegmentFirstVertex.x,
                    wallSegmentFirstVertex.y - hf + finalFirstHeight,
                    wallSegmentFirstVertex.z
                )
            );
            md.Vertices.Add(
                new Vector3(
                    wallSegmentSecondVertex.x,
                    wallSegmentSecondVertex.y - hf + finalFirstHeight,
                    wallSegmentSecondVertex.z
                )
            );
            md.Vertices.Add(
        ...
        private void CalculateEdgeList(
            MeshData md,
            UnityTile tile,
            float preferredEdgeSectionLength
        ) { }
        ...
        private void GenerateRoofMesh(MeshData md, float minHeight, float maxHeight)
        {
            ...
            switch (_options.extrusionType)
            {
                ...
                case ExtrusionType.PropertyHeight:
                    ...
                    {
                        md.Vertices[i] = new Vector3(
                            md.Vertices[i].x,
                            md.Vertices[i].y + maxHeight,
                            md.Vertices[i].z
                        );
                    }
            ...
            switch (_options.extrusionType)
            {
                ...
                case ExtrusionType.RangeHeight:
                    if (feature.Properties.ContainsKey(_options.propertyName))
                    {
                        ...
                        var featureHeight = Convert.ToSingle(
                            feature.Properties[_options.propertyName]
                        );
                        maxHeight = Math.Min(
                            Math.Max(_options.minimumHeight, featureHeight),
                            _options.maximumHeight
                        );
                        if (feature.Properties.ContainsKey("min_height"))
                        {
                            var featureMinHeight = Convert.ToSingle(
