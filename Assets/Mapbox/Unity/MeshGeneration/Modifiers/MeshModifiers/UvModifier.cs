namespace Mapbox.Unity.MeshGeneration.Modifiers
{
    ...
    public class UvModifier : MeshModifier
    {
        ...
        public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
        {
            ...
            if (
                _options.texturingType != UvMapType.Atlas
                ...
                for (int i = 0; i < _mdVertexCount; i++)
                {
                    ...
                    if (_options.style == StyleTypes.Satellite)
                    {
                        var fromBottomLeft = new Vector2(
                            (float)(
                                ((_vert.x + md.PositionInTile.x) / tile.TileScale + _size.x / 2)
                                / _size.x
                            ),
                            (float)(
                                ((_vert.z + md.PositionInTile.z) / tile.TileScale + _size.x / 2)
                                / _size.x
                            )
                        );
            ...
            {
                _currentFacade = _options.atlasInfo.Roofs[
                    UnityEngine.Random.Range(0, _options.atlasInfo.Roofs.Count)
                ];
                ...
                for (int i = 1; i < _mdVertexCount; i++)
                {
                    ...
                    _textureUvCoordinates[i] = new Vector2(
                        _vertexRelativePos.x,
                        _vertexRelativePos.z
                    );
                ...
                for (int i = 0; i < _mdVertexCount; i++)
                {
                    _uv.Add(
                        new Vector2(
                            (
                                ((_textureUvCoordinates[i].x - minx) / width)
                                * _currentFacade.TextureRect.width
                            ) + _currentFacade.TextureRect.x,
                            (
                                ((_textureUvCoordinates[i].y - miny) / height)
                                * _currentFacade.TextureRect.height
