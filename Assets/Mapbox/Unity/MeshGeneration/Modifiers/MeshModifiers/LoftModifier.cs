            ...
            foreach (var roadSegment in feature.Points)
            {
                ...
                for (int i = 0; i < _counter - 1; i++)
                {
                    for (int j = 0; j < _sliceCount - 1; j++)
                    {
                        ...
                        norm = Vector3
                            .Cross(vl[ind] - vl[ind + 1], vl[ind + _sliceCount] - vl[ind])
                            .normalized;
                        ...
                        md.Tangents.Add(
                            new Vector4(ed.normalized.x, ed.normalized.y, ed.normalized.z, 1)
                        );
                        md.Tangents.Add(
                            new Vector4(ed.normalized.x, ed.normalized.y, ed.normalized.z, 1)
                        );
                ...
                if (_closeEdges && edges.Count > 2)
                {
                    ...
                    var result = EarcutLibrary.Earcut(
                        flatData.Vertices,
                        flatData.Holes,
                        flatData.Dim
                    );
