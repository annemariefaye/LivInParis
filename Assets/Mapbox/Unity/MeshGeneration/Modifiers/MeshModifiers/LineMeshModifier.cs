    ...
    public class LineMeshModifier : MeshModifier
    {
        ...
        private void ExtrudeLine(VectorFeatureUnity feature, MeshData md)
        {
            ...
            foreach (var roadSegment in feature.Points)
            {
                ...
                {
                    _nextVertex =
                        i != (roadSegmentCount - 1)
                            ? roadSegment[i + 1]
                            : Constants.Math.Vector3Unused;
                    ...
                    var isSharpCorner =
                        cosHalfAngle < _cosHalfSharpCorner
                    ...
                    if (isSharpCorner && i > 0)
                    {
                        ...
                        if (prevSegmentLength > 2 * _sharpCornerOffset)
                        {
                            ...
                            var newPrevVertex =
                                _currentVertex - (dir * (_sharpCornerOffset / prevSegmentLength));
                    ...
                    else if (currentJoin == JoinType.Flipbevel)
                    {
                        ...
                        {
                            var direction =
                                (_prevNormal.x * _nextNormal.z - _prevNormal.z * _nextNormal.x) > 0
                                    ? -1
                                    : 1;
                            var bevelLength =
                                miterLength
                        ...
                        if (currentJoin == JoinType.Fakeround)
                        {
                            ...
                            for (var m = 0f; m < n; m++)
                            {
                                approxFractionalJoinNormal = (
                                    _nextNormal * ((m + 1f) / (n + 1f)) + (_prevNormal)
                                ).normalized;
                                AddPieSliceVertex(
                                    _currentVertex,
                                    _distance,
                            ...
                            {
                                approxFractionalJoinNormal = (
                                    _prevNormal * ((k + 1) / (n + 1)) + (_nextNormal)
                                ).normalized;
                                AddPieSliceVertex(
                                    _currentVertex,
                                    _distance,
                    ...
                    if (isSharpCorner && i < roadSegmentCount - 1)
                    {
                        ...
                        {
                            var newCurrentVertex =
                                _currentVertex
                                + (
                                    (_nextVertex - _currentVertex)
                                    * (_sharpCornerOffset / nextSegmentLength)
                                ); //._round()
        ...
        private void AddPieSliceVertex(
