namespace Mapbox.Unity.MeshGeneration.Modifiers
{
    ...
    public class ChamferHeightModifier : MeshModifier
    {
        [SerializeField]
        [Tooltip(
            "Flatten top polygons to prevent unwanted slanted roofs because of the bumpy terrain"
        )]
        ...
        [SerializeField]
        [Tooltip(
            "Fix all features to certain height, suggested to be used for pushing roads above terrain level to prevent z-fighting."
        )]
        ...
        )
        {
            ...
            meshData.Vertices.Add(
                new Vector3(
                    meshData.Vertices[originalVertexCount - 1].x,
                    meshData.Vertices[originalVertexCount - 1].y - hf,
        ...
        private static void FlattenTops(
            MeshData meshData,
            float minHeight,
            ref float hf,
            ref float max,
        ...
        public void Chamfer(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
        {
            ...
            for (int i = 0; i < feature.Points.Count; i++)
            {
                ...
                for (int j = 0; j < count; j++)
                {
                    ...
                    v1 = new Vector3(
                        md.Vertices[current].x - md.Vertices[next].x,
                        0,
                    ...
                    pij1 = new Vector3(
                        (float)(md.Vertices[next].x + n1.x),
                        0,
                        (float)(md.Vertices[next].z + n1.z)
                    );
                    pij2 = new Vector3(
                        (float)(md.Vertices[current].x + n1.x),
                        0,
                        (float)(md.Vertices[current].z + n1.z)
                    );

                    v2 = new Vector3(
                        md.Vertices[prev].x - md.Vertices[current].x,
                        0,
                    ...
                    pjk1 = new Vector3(
                        (float)(md.Vertices[current].x + n2.x),
                        0,
                        (float)(md.Vertices[current].z + n2.z)
                    );
                    pjk2 = new Vector3(
                        (float)(md.Vertices[prev].x + n2.x),
                        0,
                    ...
                    {
                        poi = new Vector3(
                            (md.Vertices[current].x + (poi - (-v1 - v2)).normalized.x),
                            0,
                    ...
                    newVertices.Add(
                        new Vector3(poi.x, poi.y + _offset + md.Vertices[current].y, poi.z)
                    );
        ...
        private List<Vector3> GetEnlargedPolygon(List<Vector3> old_points, float offset)
        {
            ...
            for (int j = 0; j < num_points; j++)
            {
                ...
                Debug.Assert(
                    lines_intersect,
                    "Edges " + i + "-->" + j + " and " + j + "-->" + k + " are parallel"
                );
        ...
        private void FindIntersection(
            ...
            out Vector3 close_p1,
            out Vector3 close_p2
        )
        {
            ...
            float t1 = ((p1.x - p3.x) * dy34 + (p3.z - p1.z) * dx34) / denominator;
