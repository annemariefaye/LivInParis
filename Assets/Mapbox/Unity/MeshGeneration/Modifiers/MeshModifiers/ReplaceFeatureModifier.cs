        ...
        public override void FeaturePreProcess(VectorFeatureUnity feature)
        {
            ...
            foreach (var point in _prefabLocations)
            {
                try
                {
                    ...
                    {
                        _featureId[index] =
                            (_featureId[index] == null) ? new List<string>() : _featureId[index];
                        _tempFeatureId = feature.Data.Id.ToString();
                        string idCandidate =
                            (_tempFeatureId.Length <= 3)
                                ? _tempFeatureId
                ...
                try
                {
                    ...
                    if (_featureId[index] != null)
                    {
                        foreach (var featureId in _featureId[index])
                        {
                            ...
                            var to = new Vector2d(
                                (feature.Points[0][0].x / feature.Tile.TileScale)
                                    + feature.Tile.Rect.Center.x,
                                (feature.Points[0][0].z / feature.Tile.TileScale)
                                    + feature.Tile.Rect.Center.y
                            );
                            ...
                            if (
                                feature
                                    .Data.Id.ToString()
                                    .StartsWith(featureId, StringComparison.CurrentCulture)
                            )
                            {
                                return true;
                            }
