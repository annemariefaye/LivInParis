...
namespace Mapbox.Unity.MeshGeneration.Factories
{
    ...
    public class VectorTileFactory : AbstractTileFactory
    {
        ...
        {
            get { return _properties.sourceOptions.Id; }
        ...
        {
            CreateFeatureWithBuilder(
                tile,
                visualizer.SubLayerProperties.coreOptions.layerName,
                visualizer
            );
        }
        ...
        public virtual LayerVisualizerBase AddVectorLayerVisualizer(
        ...
        )
        {
            ...
            bool isPrimitiveTypeValidForBuidingIds =
            ...
            bool isSourceValidForBuildingIds =
                _properties.sourceType != VectorSourceType.MapboxStreets;

            subLayer.honorBuildingIdSetting =
        ...
        public virtual LayerVisualizerBase AddPOIVectorLayerVisualizer(
        ...
        )
        {
            ...
            ((LocationPrefabsLayerVisualizer)visualizer).SetProperties(
                (PrefabItemOptions)poiSubLayer
            );
        ...
        public virtual LayerVisualizerBase FindVectorLayerVisualizer(
            VectorSubLayerProperties subLayer
        )
        {
            if (_layerBuilder.ContainsKey(subLayer.Key))
            {
                var visualizer = _layerBuilder[subLayer.Key]
                    .Find((obj) => obj.SubLayerProperties == subLayer);
                return visualizer;
            }
            return null;
        }
        ...
        public virtual void RemoveVectorLayerVisualizer(LayerVisualizerBase subLayer)
        {
            ...
            if (_layerBuilder.ContainsKey(subLayer.Key))
            {
                if (Properties.vectorSubLayers.Contains(subLayer.SubLayerProperties))
                {
                    ...
                    subLayer.SubLayerProperties is PrefabItemOptions
                    && Properties.locationPrefabList.Contains(
                        subLayer.SubLayerProperties as PrefabItemOptions
                    )
                )
                {
                    Properties.locationPrefabList.Remove(
                        subLayer.SubLayerProperties as PrefabItemOptions
                    );
                }
        ...
        protected override void OnRegistered(UnityTile tile)
        {
            if (
                ...
                || _properties.sourceOptions.isActive == false
                || (_properties.vectorSubLayers.Count + _properties.locationPrefabList.Count) == 0
            )
            {
        ...
        {
            if (
                tile.VectorDataState != TilePropertyState.Unregistered
                && tile.RasterDataState != TilePropertyState.Loading
                && tile.HeightDataState != TilePropertyState.Loading
            )
            {
        ...
        private void OnDataError(
            UnityTile tile,
            Mapbox.Map.VectorTile vectorTile,
        ...
        private void TrackFeatureWithBuilder(
            UnityTile tile,
            string layerName,
            LayerVisualizerBase builder
        )
        {
            if (builder.Active)
            {
        ...
        private void CreateFeatureWithBuilder(
            UnityTile tile,
            string layerName,
            LayerVisualizerBase builder
        )
        {
            if (builder.Active)
            {
                ...
                else
                {
                    ...
                    builder.Create(
                        tile.VectorData.Data.GetLayer(tile.VectorData.Data.LayerNames()[0]),
                        tile,
                        DecreaseProgressCounter
                    );
                }
