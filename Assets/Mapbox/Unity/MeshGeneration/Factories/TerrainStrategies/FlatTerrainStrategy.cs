...
namespace Mapbox.Unity.MeshGeneration.Factories.TerrainStrategies
{
    public class FlatTerrainStrategy : TerrainStrategy
    {
        ...
        public override void RegisterTile(UnityTile tile)
        {
            ...
            if (
                tile.RasterDataState != Enums.TilePropertyState.Loaded
        ...
        private void BuildQuadWithSides(UnityTile tile)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                verts[4 * (i + 1) + 2] =
                    verts[i] + new Vector3(0, -_elevationOptions.sideWallOptions.wallHeight, 0);
                verts[4 * (i + 1) + 3] =
                    verts[i + 1] + new Vector3(0, -_elevationOptions.sideWallOptions.wallHeight, 0);

                norm = Vector3
                    .Cross(
                        verts[4 * (i + 1) + 1] - verts[4 * (i + 1) + 2],
                        verts[4 * (i + 1)] - verts[4 * (i + 1) + 1]
                    )
                    .normalized;
                norms[4 * (i + 1)] = norm;
                norms[4 * (i + 1) + 1] = norm;
