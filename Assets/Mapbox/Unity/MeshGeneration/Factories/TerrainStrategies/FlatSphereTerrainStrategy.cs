...
namespace Mapbox.Unity.MeshGeneration.Factories.TerrainStrategies
{
    public class FlatSphereTerrainStrategy : TerrainStrategy
    {
        public float Radius
        {
            get { return _elevationOptions.modificationOptions.earthRadius; }
        }
        ...
        public override void RegisterTile(UnityTile tile)
        {
            ...
            if (
                (int)tile.ElevationType != (int)ElevationLayerType.GlobeTerrain
        ...
        void GenerateTerrainMesh(UnityTile tile)
        {
            ...
            for (float x = 0; x < _sampleCount; x++)
            {
                ...
                {
                    var xx = Mathf.Lerp(
                        (float)tile.Rect.Min.x,
                        ((float)tile.Rect.Min.x + (float)tile.Rect.Size.x),
                        x / (_sampleCount - 1)
                    );
                    var yy = Mathf.Lerp(
                        (float)tile.Rect.Max.y,
                        ((float)tile.Rect.Max.y + (float)tile.Rect.Size.y),
                        y / (_sampleCount - 1)
                    );
