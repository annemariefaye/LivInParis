...
namespace Mapbox.Unity.MeshGeneration.Factories.TerrainStrategies
{
    public class LowPolyTerrainStrategy : TerrainStrategy, IElevationBasedTerrainStrategy
    {
        ...
        public override void RegisterTile(UnityTile tile)
        {
            ...
            if (
                (int)tile.ElevationType != (int)ElevationLayerType.LowPolygonTerrain
            ...
            for (float y = 0; y < cap; y++)
            {
                for (float x = 0; x < cap; x++)
                {
                    var x1 =
                        tile.TileScale
                        * (float)(
                            Mathd.Lerp(tile.Rect.Min.x, tile.Rect.Max.x, x / cap)
                            - tile.Rect.Center.x
                        );
                    var y1 =
                        tile.TileScale
                        * (float)(
                            Mathd.Lerp(tile.Rect.Min.y, tile.Rect.Max.y, y / cap)
                            - tile.Rect.Center.y
                        );
                    var x2 =
                        tile.TileScale
                        * (float)(
                            Mathd.Lerp(tile.Rect.Min.x, tile.Rect.Max.x, (x + 1) / cap)
                            - tile.Rect.Center.x
                        );
                    var y2 =
                        tile.TileScale
                        * (float)(
                            Mathd.Lerp(tile.Rect.Min.y, tile.Rect.Max.y, (y + 1) / cap)
                            - tile.Rect.Center.y
                        );
