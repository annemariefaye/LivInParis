...
namespace Mapbox.Unity.MeshGeneration.Factories.TerrainStrategies
{
    public class ElevatedTerrainWithSidesStrategy : TerrainStrategy, IElevationBasedTerrainStrategy
    {
        ...
        public override int RequiredVertexCount
        {
            get
            {
                return _elevationOptions.modificationOptions.sampleCount
        ...
        {
            if (
                _elevationOptions.unityLayerOptions.addToLayer
        ...
        private void CreateBaseMesh(UnityTile tile)
        {
            ...
            for (float y = 0; y < _sampleCount; y++)
            {
                ...
                for (float x = 0; x < _sampleCount; x++)
                {
                    ...
                    _newUvList.Add(
                        new Vector2(x * 1f / (_sampleCount - 1), 1 - (y * 1f / (_sampleCount - 1)))
                    );
                }
            }
        ...
        private void GenerateTerrainMesh(UnityTile tile)
        {
            ...
            for (float y = 0; y < _sampleCount; y++)
            {
                for (float x = 0; x < _sampleCount; x++)
                {
                    ...
                    _currentTileMeshData.Normals[(int)(y * _sampleCount + x)] = Mapbox
                        .Unity
                        .Constants
                        .Math
                        .Vector3Zero;
            ...
            for (int y = 0; y < _sampleCount - 1; y++)
            {
                for (int x = 0; x < _sampleCount - 1; x++)
                {
                    ...
                    _newDir = Vector3.Cross(
                        _currentTileMeshData.Vertices[_vertB]
                            - _currentTileMeshData.Vertices[_vertA],
                        _currentTileMeshData.Vertices[_vertC]
                            - _currentTileMeshData.Vertices[_vertA]
                    );
        ...
        private void FixStitches(UnwrappedTileId tileId, MeshData mesh)
        {
            ...
            if (_stitchTarget != null)
            {
                ...
                for (int i = 0; i < _sampleCount; i++)
                {
                    ...
                    mesh.Vertices[meshVertCount + 6 + (8 * i)] = mesh.Vertices[
                        meshVertCount - _sampleCount + i
                    ];
            ...
            if (_stitchTarget != null)
            {
                ...
                for (int i = 0; i < _sampleCount; i++)
                {
                    ...
                    mesh.Vertices[meshVertCount + 4 + (8 * i)] = mesh.Vertices[
                        i * _sampleCount + _sampleCount - 1
                    ];
