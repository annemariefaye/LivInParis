...
namespace Mapbox.Unity.Map.TileProviders
{
    ...
    public class QuadTreeMapVisualizer : AbstractMapVisualizer
    {
        protected override void PlaceTile(UnwrappedTileId tileId, UnityTile tile, IMapReadable map)
        {
            ...
            UnwrappedTileId centerTile = TileCover.CoordinateToTileId(
                map.CenterLatitudeLongitude,
                map.AbsoluteZoom
            );
            //get center WebMerc corrdinates of tile covering the center (Unity 0,0,0)
            Vector2d centerTileCenter = Conversions.TileIdToCenterWebMercator(
                centerTile.X,
                centerTile.Y,
                map.AbsoluteZoom
            );
            ...
            float factor =
                Conversions.GetTileScaleInMeters(0f, map.AbsoluteZoom) * 256.0f / unityTileSize;
