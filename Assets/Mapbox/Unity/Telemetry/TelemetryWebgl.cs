namespace Mapbox.Unity.Telemetry
{
    ...
    public class TelemetryWebgl : ITelemetryLibrary
    {
        ...
        {
            _url = string.Format(
                "{0}events/v2?access_token={1}",
                Mapbox.Utils.Constants.EventsAPI,
                accessToken
            );
        }
        ...
        public void SendTurnstile()
        {
            ...
            if (ShouldPostTurnstile(ticks))
            {
                ...
                PlayerPrefs.SetString(
                    Constants.Path.TELEMETRY_TURNSTILE_LAST_TICKS_FALLBACK_KEY,
                    ticks.ToString()
                );
            }
        }
        ...
        bool ShouldPostTurnstile(long ticks)
        {
            ...
            var lastDateString = PlayerPrefs.GetString(
                Constants.Path.TELEMETRY_TURNSTILE_LAST_TICKS_FALLBACK_KEY,
                longAgo
            );
            long lastTicks = 0;
