        ...
        private void LoadItemData()
        {
            ...
            for (int i = 0; i < donneesData.Count; i++)
            {
                ...
                double longitude = double.Parse(
                    donneesData[i]["Longitude"].ToString(),
                    System.Globalization.CultureInfo.InvariantCulture
                );
                double latitude = double.Parse(
                    donneesData[i]["Latitude"].ToString(),
                    System.Globalization.CultureInfo.InvariantCulture
                );
                ...
                stationDataList.Add(
                    new ItemStationData(id, ligne, libelle, longitude, latitude, commune, codeInsee)
                );
            }
            ...
            for (int i = 0; i < donneesGraphe.Count; i++)
            {
                ...
                stationGrapheList.Add(
                    new ItemStationGraphe(
                        stationId,
                        stationNom,
                        precedentId,
                        suivantId,
                        tempsEntreDeuxStations,
                        tempsDeChangement,
                        sensUnique
                    )
                );
